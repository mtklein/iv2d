...
static enum {NONE,FULL,MAYBE} coverage(iv R) {
    assert(R.lo <= R.hi);
    if (R.hi <  0) { return FULL; } // [-a, -b]                think, "all negative"
    if (R.lo >= 0) { return NONE; } // [+a, +b] or [ 0, +b] or [0,0]  "non-negative"
    assert(R.lo < 0 && 0 <= R.hi);  // [-a, +b] or [-a,  0]          "some negative"
    return MAYBE;
}

static float cover_subpixel(float l, float t, float r, float b,
                            iv const R, int limit, struct iv2d_region const *region) {
    // We may assume cover_subpixel() is only called on areas with uncertain coverage.
    assert(coverage(R) == MAYBE);
    ...
    if (limit == 0) {
        ...
        // But we can do a whole lot better---some might say unreasonably so---by examining R.
        ...
        #else
            // coverage(R) == MAYBE implies R.lo < 0 <= R.hi, so
            //   1) R.lo        is non-zero
            //   2) R.hi - R.lo is non-zero
...
void iv2d_cover(struct iv2d_rect   const  bounds,
    ...
    int const l = bounds.l,
              ...
              b = bounds.b;
    if (l < r && t < b) {
        iv const X = {(float)l, (float)r},
                 Y = {(float)t, (float)b},
                 R = region->fn(region, X,Y);
