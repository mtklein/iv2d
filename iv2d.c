...
static float4 estimate_coverage_(struct iv2d_region const *region, void *scratch,
                                 float l, float t, float r, float b, int limit) {
    // Evaluate LT, LB, RT, and RB corners of the region.
    float const x = (l+r)/2,
                y = (t+b)/2;
    iv const corners = region->eval(region,scratch, (iv){{l,l,x,x}, {x,x,r,r}}
                                                  , (iv){{t,y,t,y}, {y,b,y,b}});
    ...
    if (--limit) {
        if (uncertain[0]) { cov += estimate_coverage_(region,scratch, l,t,x,y, limit); }
        if (uncertain[1]) { cov += estimate_coverage_(region,scratch, l,y,x,b, limit); }
        if (uncertain[2]) { cov += estimate_coverage_(region,scratch, x,t,r,y, limit); }
        if (uncertain[3]) { cov += estimate_coverage_(region,scratch, x,y,r,b, limit); }
    } else {
...
static float estimate_coverage(struct iv2d_region const *region, void *scratch,
                               float l, float t, float r, float b, int limit) {
    float4 const cov = estimate_coverage_(region,scratch, l,t,r,b, limit);
    return cov[0]+cov[1]+cov[2]+cov[3];
}

static void iv2d_cover_(struct iv2d_region const *region, void *scratch,
    ...
    if (l<r && t<b) {
        ...
        if (__builtin_reduce_or(uncertain)) {
            if (r-l <= 1 && b-t <= 1) {  // These floats hold integers, so actually compare ==.
                if (quality > 0) {
                    float const cov = estimate_coverage(region,scratch, l,t,r,b, quality);
                    if (cov > 0) {
                ...
                if (not_outside[0]) { iv2d_cover_(region,scratch, l,t,x,y, quality, yield,ctx); }
                if (not_outside[1]) { iv2d_cover_(region,scratch, l,y,x,b, quality, yield,ctx); }
                if (not_outside[2]) { iv2d_cover_(region,scratch, x,t,r,y, quality, yield,ctx); }
                if (not_outside[3]) { iv2d_cover_(region,scratch, x,y,r,b, quality, yield,ctx); }
            }
        }
    }
}
void iv2d_cover(struct iv2d_region const *region,
                ...
                int quality,
                void (*yield)(void*, float,float,float,float, float), void *ctx) {
    void *scratch = calloc(1, region->scratch);
    iv2d_cover_(region,scratch, (float)l, (float)t, (float)r, (float)b, quality, yield,ctx);
    free(scratch);
}
